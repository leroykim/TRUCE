...
class QueryFactory:
    def __init__(self, form: FlaskForm, data_class: str):
        ...
        self.variables["observation"] = get_variables(form, get_observation_selections)
    ...
    def get_federated_query(self, endpoint_list):
        ...
        prefix = get_prefix()
        select = get_select_clause(variables=self.variables, additional_variables=["veracity_weighted_average"])
...
def get_triples(variables: dict[str, list], condition_query: str = None) -> str:
    ...
    for datatype, variable_list in variables.items():
        if variable_list:
            ...
            triple_list.append(f"?{datatype.lower()} a syn:{datatype.capitalize()} .")
            triple_list.extend([f"?{datatype.lower()} syn:{i} ?{i} ." for i in variable_list])
    if condition_query:
...
def get_federated_clause(
    endpoints: list, variables: dict[str, list], condition_query: str = None, policies: list = None
):
