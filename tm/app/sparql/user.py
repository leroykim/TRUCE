...
class UserInfo:
    ...
    def __get_trust_where_pattern(self):
        ...
        where_pattern.add_triples(
            triples=[
                Triple(
                    subject=self.individual_id,
                    predicate="a",
                    object=f"syn:{self.user_type}",
                ),
                Triple(
                    subject=self.individual_id,
                    predicate="tst:identity",
                    object="?identity_trust",
                ),
                Triple(
                    subject=self.individual_id,
                    predicate="tst:behavior",
                    object="?behavioral_trust",
                ),
            ]
        )
    ...
    def __get_trust_score(self):
        ...
        user_info = self.fuseki.query_with_time(query, format="json")
    ...
    def __get_trust_query(self):
        ...
        select_query.add_prefix(
            Prefix(
                prefix="syn",
                namespace="https://knacc.umbc.edu/leroy/ontologies/synthea#",
            )
        )
        select_query.add_prefix(
            Prefix(
                prefix="tst", namespace="https://knacc.umbc.edu/leroy/ontologies/trust#"
            )
        )
