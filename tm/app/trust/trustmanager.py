...
class TrustManager:
    ...
    def update(self, user_id: str, policy_result: dict[str, bool]):
        st = time.time()
        currunt_behavior_trust = self.get_behavior_trust_score(user_id=user_id)
    ...
    def update_credibility(self, policy_result: dict[str, bool]):
        st = time.time()
        currunt_credibility = self.get_credibility_score()
    ...
    def get_behavior_trust_score(self, user_id: str) -> float:
        ...
        result = self.fuseki.query_with_time(
            sparql_query=select_query.get_text(), format="json"
        )
    ...
    def get_identity_trust_score(self, user_id: str) -> float:
        current_app.logger.info(f"Getting {user_id}'s identityTrust...")
        select_query = SPARQLSelectQuery(distinct=True)
        triples = [
            Triple(
                subject="?user",
        ...
        pattern.add_triples(triples=triples)
        select_query.set_where_pattern(pattern)
        select_query.add_variables(variables=["?identityTrust"])
        for prefix in self.prefix_list:
            select_query.add_prefix(prefix=prefix)
        result = self.fuseki.query_with_time(
            sparql_query=select_query.get_text(), format="json"
        )
        identity_trust = result["results"]["bindings"][0]["identityTrust"]["value"]
        current_app.logger.info(f"{user_id}'s identityTrust: {identity_trust}")
        return float(identity_trust)
